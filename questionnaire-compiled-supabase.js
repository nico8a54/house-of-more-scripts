(function () {
  ...
  const WORKER_URL = "https://houseofmore.nico-97c.workers.dev";
    ...
    if (yesOption && noOption && attendedEvents && eventsRadios.length) {
      yesOption.addEventListener("change", () => {
        if (yesOption.checked) {
          ...
          eventsRadios.forEach(r => r.setAttribute("required", ""));
        }
      });
      noOption.addEventListener("change", () => {
        if (noOption.checked) {
          attendedEvents.classList.add("hide");
          eventsRadios.forEach(r => r.removeAttribute("required"));
        }
      });
    }
  });
  ...
  /*=========================================================
    SECTION 2 — FORM SUBMIT → WORKER → SUPABASE
  =========================================================*/
  document.addEventListener("DOMContentLoaded", () => {
    const form   = document.querySelector('[data-ms-code="questionnaire-form"]');
    const button = document.querySelector("#submit");

    if (!form)   { console.error("[SUPABASE] Form not found");   return; }
    if (!button) { console.error("[SUPABASE] Submit button not found"); return; }
    ...
    button.addEventListener("click", async () => {
      ...
      try {
        ...
        // 3. Memberstack identity from DOM
        payload.member_id = document.querySelector('[data-ms-member="id"]')?.textContent?.trim();
        payload.email     = document.querySelector('[data-ms-member="email"]')?.textContent?.trim();

        if (!payload.member_id) throw new Error("No member_id found — is the user logged in?");

        console.log("[SUPABASE] Payload:", payload);

        // 4. Send to Worker → Supabase
        const response = await fetch(`${WORKER_URL}/questionnaire-supabase`, {
          method: "POST",
        ...
        console.log("[SUPABASE] Successfully written");
